  ...
  return (
    <div className="relative w-full flex items-center">
      <button onClick={() => scroll("left")} className="p-2 hidden md:block">
        <IoChevronBack className="w-8 h-8 text-light-accent hover:text-warm-accent" />
      </button>
      <div
        ref={scrollRef}
        className="flex gap-4 overflow-x-auto scroll-smooth 
          scrollbar-thin scrollbar-thumb-warm-accent scrollbar-track-dark-gray
          w-full px-4">
        {skills.map((skill, index) => {
          ...
          return (
            <div
              ...
              <span className="font-bold text-light-accent text-lg mb-2">
                {skill.name}
              </span>
      ...
      <button onClick={() => scroll("right")} className="p-2 hidden md:block">
        <IoChevronForward className="w-8 h-8 text-light-accent hover:text-warm-accent" />
      </button>
    </div>
  );
}
